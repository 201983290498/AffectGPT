...
class Blip2Base(BaseModel):
    ...
    # Blip2的视觉编码器由 vit + ln 组成
    @classmethod
    def init_vision_encoder(
        cls, model_path, img_size, drop_path_rate, use_grad_checkpoint, precision
    ):
    ...
    def load_from_pretrained(self, url_or_filename):
        if is_url(url_or_filename):
            ...
            # checkpoint = torch.load(url_or_filename, map_location="cpu") # 很有意思，所有模型参数一开始都是加载到cpu中的
...
class LayerNorm(nn.LayerNorm):
    ...
    # 假设输入是torch.float16，但是nn.LayerNorm只支持32，因此中间需要一个参数转换的过程
